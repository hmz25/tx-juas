...
#analyze tree orthomosaic images by combining cone counts from quadrats


### assemble cones per tree ########################################################
...
tree_cones <- quadrat_cones %>% 
  group_by(date, site, tree) %>% 
  summarize(tree_cones_mean = mean(total_cones, na.rm = TRUE),
            tree_cones_sd = sd(total_cones,  na.rm = TRUE),
            tree_cones_n = n())




### extract indices from orthomosaics of trees #######################################################

### create a classification model to mask non foliage and non cones
...
### orthoimages of focal trees

#load image files
tree_ortho_files <- list.files("C:/Users/dsk273/Box/texas/pollen_production/TX jan 24/ortho_treetop_segmented/cropped", full = TRUE)
photo_i <- stack(tree_ortho_files[32])
plotRGB(photo_i)
...
### apply model to all trees and save in a new folder
photo_dir <- "C:/Users/dsk273/Box/texas/pollen_production/TX jan 24/ortho_treetop_segmented/cropped"
photo_list <- list.files(photo_dir, full.names = FALSE)
photo_list_full_dir <- list.files(photo_dir, full.names = TRUE)


for(i in 13:length(photo_list)){
  photo_i <- stack(photo_list_full_dir[i])  #plotRGB(photo_i)
  ...
  save_file_name = paste0("C:/Users/dsk273/Box/texas/pollen_production/TX jan 24/ortho_treetop_segmented/cropped_masked/",photo_list[i])
...
photo_dir <- "C:/Users/dsk273/Box/texas/pollen_production/TX jan 24/ortho_treetop_segmented/cropped_masked/"
photo_list <- list.files(photo_dir, full.names = FALSE)
photo_list_full_dir <- list.files(photo_dir, full.names = TRUE)
...
for(i in 1:length(photo_list)){
  photo_i <- stack(photo_list_full_dir[i]) 
  
  
  #photo_i <- stack(photo_list_full_dir[31]) 
  #plot(photo_i) 
  #plotRGB(photo_i)

  photo_i[[1]][photo_i[[4]] == 1] <- NA
  photo_i[[2]][photo_i[[4]] == 1] <- NA
  photo_i[[3]][photo_i[[4]] == 1] <- NA
  ...
  photo_i_index_rg_thresh[photo_i_index_rg_dif[] < 0.0] <- 0
  photo_i_index_rg_thresh[photo_i_index_rg_dif[] > 0.0] <- 1
  ...
  photo_i_r_m <- cellStats(photo_i[[1]], "mean")
  photo_i_g_m <- cellStats(photo_i[[2]], "mean")
  photo_i_b_m <- cellStats(photo_i[[3]], "mean")
  photo_i_mask <- cellStats(photo_i[[4]], "mean") - 1 #1 == background, 2 = foreground
  
  photo_i_rgb <- data.frame(photo_list[i], photo_i_r_m, photo_i_g_m, photo_i_b_m, photo_i_mask, photo_i_rg_dif, 
...
male_trees <- photo_masked_df %>% 
  mutate(site = substr(photo_list.i., 1, 4),
         tree_raw = stringr::str_sub(photo_list.i., -6, -4),
         tree = gsub("[^0-9-]", "", tree_raw),
         tree_sex = case_when(grepl("_", tree_raw) ~ "female",
                              grepl("t", tree_raw) ~ "male" )) %>% 
  filter(tree_sex == "male")

female_trees <- photo_masked_df %>% 
  mutate(site = substr(photo_list.i., 1, 4),
         tree_raw = stringr::str_sub(photo_list.i., -6, -4),
         tree = gsub("[^0-9-]", "", tree_raw),
         tree_sex = case_when(grepl("_", tree_raw) ~ "female",
                              grepl("t", tree_raw) ~ "male" )) %>% 
  filter(tree_sex == "female")


tree_cones_join <- tree_cones %>% 
  mutate(tree_sex = "male",
         tree = as.character(tree))

tree_cones_orth <- left_join(tree_cones_join, male_trees)
tree_cones_orth <- bind_rows(tree_cones_orth, female_trees) %>% 
  mutate(tree_cones_mean = case_when(tree_sex == "male" ~ tree_cones_mean,
                                      tree_sex == "female" ~ 0))

#
# tree_cones_rgb <- left_join(tree_cones, photo_masked_df_join) %>%
#   filter(!is.na(total_cones)) %>%
#   filter(!is.na(photo_i_r)) %>% mutate(photo_i_mask = photo_i_mask - 1)

# tree_cones_rgb_test <- tree_cones_rgb %>%  
#   filter((photo_i_r + photo_i_g + photo_i_b) > 0)

# tree_cones_rgb_test <- tree_cones_rgb_test %>% filter(photo_i_mask > 0.5)

tree_cones_orth %>% 
  mutate(spectral_index = photo_i_rg_dif) %>% 
ggplot(aes(x= spectral_index  , y = tree_cones_mean)) + geom_point(alpha = 0.5) + theme_bw() + geom_smooth(method = "lm", se = FALSE) +
  xlab("spectral index") + ylab(cone~density~(cones/m^2)) + ggthemes::theme_few()#scale_color_viridis_c()  # + facet_wrap(~site)


fit <- lm(tree_cones_orth$tree_cones_mean  ~ tree_cones_orth$photo_i_rg_dif   )
summary(fit)

#fit$residuals
tree_cones_orth  <- tree_cones_orth %>%
  mutate(photo_i_rg_dif_resid = fit$residuals,
         photo_i_rg_dif_pred = fit$fitted.values)



cor(tree_cones_rgb_test)
tree_cones_rgb_test %>% dplyr::select(10:23, - photo_list.i.) %>% 
