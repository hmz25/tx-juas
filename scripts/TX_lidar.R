...
folders <- list.dirs(path="./rs_data", recursive = F)# folders with each dataset
sites <- st_read("./qgis/TX_sites.gpkg")# loading sites
sites <- st_buffer(sites,20)# 20m buffer to avoid cutting trees on on the sites
files_for_crs <- list.files(path = "./rs_data/north_central", pattern = ".laz$", full.names = T, recursive = TRUE) #loading one of datasets (just for correct crs)
...
### result should be 9 .las files ready to segmentation
