...
#create data frame with index values
index_df <- quadrat_px_df |>
  mutate(orange_index = (r - g) / (r + g)) |> #add index
  group_by(site, tree) |>
  summarize(mean_index = mean(orange_index, na.rm = TRUE)) |> #summarize index values for each tree at each site 
  mutate(site = substr(site, 1, 4))
...
#create data frame with calculated "total cones" from manual processing
total_cone_df <- cone_df_clean |> 
  mutate(cones_per_g = count/weight) |> #create column of cone count/weight
  group_by(date_collected, tree, site, total_mass) |> 
  summarize(mean_cones_per_g = mean(cones_per_g)) |> #take mean of manual counts/weight
  mutate(total_cones = mean_cones_per_g*total_mass) |> #multiply by total weight of sample
  mutate(tree = as.character(tree))

cone_index_df <- left_join(index_df, total_cone_df) #resulting df with cone counts and index values for each tree

cone_px_df <- left_join(quadrat_px_df, total_cone_df) #resulting df with cone counts and pixel values for each tree 

#join df with manual pheno data
fieldmaps_df <- read_csv('/Users/hannahzonnevylle/Library/CloudStorage/Box-Box/Katz lab/texas/focal_trees_2025.csv') #pheno data from field maps 
...
ggplot(df_long, aes(x = value, fill = color_channel)) +
  ...
  scale_fill_manual(values = c("r" = "red", "g" = "forestgreen", "b" = "blue")) +
  theme_classic() +
