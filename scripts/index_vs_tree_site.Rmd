---
title: "how does spectral index vs cone density change based on site and tree?"
output: pdf_document
date: "2024-11-14"
---
...
#shit this was done using orthos but do we actually want to apply it to the pics of the quadrats? 
...
TESTING WORKFLOW ON ONE SITE, NEED TO SCALE UP TO USING ALL SITES 
...
mask out everything from ortho except for focal trees using the shapefiles
```{r}
...
run pixel classifier to filter out everything that is not foliage 
```{r}
...
wade_ortho_ttops_rgb <- subset(wade_ortho_ttops, 1:3)

#ensure names in rf df and ortho raster are the same
names(wade_ortho_ttops_rgb) <- colnames(training_df)[1:3]

#use rf to filter out everything that isn't foliage
wade_ortho_ttops_rf <- predict(wade_ortho_ttops_rgb, rf_mask) #this took a long time, I kind of think it's still looking thru the entire tiff?
plot(wade_ortho_ttops_rf)

#combine rgb ortho RasterStack with rf mask RasterStack
wade_ortho_ttops_rf_rgb <- c(wade_ortho_ttops_rgb, wade_ortho_ttops_rf)
# print(wade_ortho_ttops_rf_rgb)
# plot(wade_ortho_ttops_rf_rgb)

#create mask layer where only pixels that fall into "yes" class are retained

mask_layer <- ifel(wade_ortho_ttops_rf_rgb$class == "yes", 1, NA)
#plot(mask_layer)

#filter rgb ortho with mask layer to only retain pixel values that are foliage 
wade_ortho_ttops_filtered <- mask(wade_ortho_ttops_rf_rgb, mask_layer)
#plot(wade_ortho_ttops_filtered)
```


extract RGB values from the filtered orthos and their corresponding geometry IDs, compile into df 
```{r}
wade_ortho_ttops_filtered_df <- terra::extract(wade_ortho_ttops_filtered, wade_shp_reproj, df = TRUE)
# plotRGB(wade_ortho_ttops_filtered)[1]
# plot(wade_shp_reproj, add = TRUE)
#str(wade_ortho_ttops_filtered_df)
#tail(wade_ortho_ttops_filtered_df)
```


rename columns with red, green, blue bands and so that ID = tree #. Also add index column 
```{r}
wade_ortho_ttops_index_df <- wade_ortho_ttops_filtered_df %>% 
  dplyr::select(1:4) %>% 
  rename(tree = ID, r = 2, g = 3, b = 4) %>% 
  mutate(orange_index = ((r-g)/(r+g))) %>% 
  mutate(site = "wade") %>% 
  group_by(site, tree) %>% 
  summarize(sum_index = abs(sum(orange_index, na.rm = TRUE)))
```
...
connect pixel values w cone density data
```{r}
