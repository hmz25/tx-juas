...
rf_mask_adj <- get(load("Katz lab/texas/rf_mask_adj.RData"))
...
photo_j_adj <- rast("Katz lab/texas/tx 2026 drone pics/2026 quadrat pics/cropped_quadrat_pics/wade_t3_tree_adj.tif")
plotRGB(photo_j_adj)
names(photo_j_adj) <- c("r", "g", "b")
photo_j_adj_mask <- predict(photo_j_adj, rf_mask_adj)
plot(photo_j_adj_mask)
...
for(i in seq_along(photo_list)){
  ...
  #if basename contains "_adj", apply rf trained on adjusted pixels. if not, apply regular rf
  if (str_detect(basename(photo_list_full_dir[i]), "_adj")) {
    photo_i_mask_df <- predict(rf_mask_adj, photo_i_df)
  } else {
