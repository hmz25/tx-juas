...
library(readr)
quadrat_cones <- read_csv("~/Desktop/juas/2024 analysis/cone processing 24 - counts.csv")

quadrat_cones <- quadrat_cones %>%
  mutate(across(c("s1_count", "s1_weight", "s2_count", "s2_weight", "s3_count", "s3_weight", "s4_count", "s4_weight", "s5_count", "s5_weight"), as.numeric))
...
list.files("~/Desktop/juas/2024 analysis/cropped eye levels") #83 files

eyelevel_files <- list.files("~/Desktop/juas/2024 analysis/cropped eye levels")#[1]
image_directory <- "~/Desktop/juas/2024 analysis/cropped eye levels/"
...
#checking the code 
process_eyelevel_images(list.files("~/Desktop/juas/2024 analysis/cropped eye levels")[15]) #pulling up file numbers named in the function to check if it works 
...
# library(openxlsx)
# write.xlsx(avg_pixel_cone_density, "~/Desktop/juas/2024 analysis")
